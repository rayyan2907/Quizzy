...
namespace Quizzy.Models.Data_Layer.quiz
{
    public class stu_quizDL
    {
        ...
        {
            string query = $"select quizID,isPublic,quiz_name,given_time,s.subject_name,s.subject_code,q.subjectID,is_asssign from quiz q join subjects s on s.subjectID=q.subjectID  where isPublic=true;";
            return DatabaseHelper.Instance.GetData(query);
        }
    }
}
