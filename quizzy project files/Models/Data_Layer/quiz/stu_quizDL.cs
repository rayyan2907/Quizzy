...
namespace Quizzy.Models.Data_Layer.quiz
{
    public class stu_quizDL
    {
        ...
        public static DataTable getResults(string subid,string stuid)
        {
            string query = $"select quiz_name, resultID, mcq_marks, shq_marks, total_marks, q.quizID, studentID,q.subjectID from results r join quiz q on q.quizID=r.quizID where r.studentID = {stuid} and subjectID={subid}";
            return DatabaseHelper.Instance.GetData(query);
        }
